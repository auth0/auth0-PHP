...
final class SdkConfiguration implements ConfigurableContract
{
     ...
     * @param null|string                      $cookieSecret                    The secret used to derive an encryption key (via HKDF-SHA256) for the user identity in a session cookie and to sign the transient cookies used by the login callback. Should be at least 32 characters.
    ...
    public function setCookieSecret(?string $cookieSecret = null): self
    {
        ...
        if (null !== $cookieSecret && mb_strlen($cookieSecret) < 32) {
            @trigger_error(
