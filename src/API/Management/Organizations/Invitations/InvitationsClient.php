...
use Auth0\SDK\API\Management\Types\ListOrganizationInvitationsOffsetPaginatedResponseContent;
...
class InvitationsClient implements InvitationsClientInterface
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    public function list(string $id, ListOrganizationInvitationsRequestParameters $request = new ListOrganizationInvitationsRequestParameters(), ?array $options = null): Pager
    {
        return new OffsetPager(
            request: $request,
            getNextPage: fn (ListOrganizationInvitationsRequestParameters $request) => $this->_list($id, $request, $options),
            /* @phpstan-ignore-next-line */
            getOffset: fn (ListOrganizationInvitationsRequestParameters $request) => $request?->getPage() ?? 0,
    ...
    public function create(string $id, CreateOrganizationInvitationRequestContent $request, ?array $options = null): ?CreateOrganizationInvitationResponseContent
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return CreateOrganizationInvitationResponseContent::fromJson($json);
            }
    ...
    public function get(string $id, string $invitationId, GetOrganizationInvitationRequestParameters $request = new GetOrganizationInvitationRequestParameters(), ?array $options = null): ?GetOrganizationInvitationResponseContent
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return GetOrganizationInvitationResponseContent::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new Auth0Exception(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     *   bodyProperties?: array<string, mixed>,
     * } $options
     * @return ?ListOrganizationInvitationsOffsetPaginatedResponseContent
     * @throws Auth0Exception
     * @throws Auth0ApiException
     */
    private function _list(string $id, ListOrganizationInvitationsRequestParameters $request = new ListOrganizationInvitationsRequestParameters(), ?array $options = null): ?ListOrganizationInvitationsOffsetPaginatedResponseContent
    {
        $options = array_merge($this->options, $options ?? []);
        $query = [];
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListOrganizationInvitationsOffsetPaginatedResponseContent::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new Auth0Exception(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
