...
use Auth0\SDK\API\Management\Types\GetGroupMembersResponseContent;
...
class MembersClient implements MembersClientInterface
{
    ...
    public function get(string $id, GetGroupMembersRequestParameters $request = new GetGroupMembersRequestParameters(), ?array $options = null): Pager
    {
        return new CursorPager(
            request: $request,
            getNextPage: fn (GetGroupMembersRequestParameters $request) => $this->_get($id, $request, $options),
            setCursor: function (GetGroupMembersRequestParameters $request, ?string $cursor) {
                $request->setFrom($cursor);
            },
            /* @phpstan-ignore-next-line */
            getNextCursor: fn (?GetGroupMembersResponseContent $response) => $response?->getNext() ?? null,
            /* @phpstan-ignore-next-line */
            getItems: fn (?GetGroupMembersResponseContent $response) => $response?->getMembers() ?? [],
        );
    }
     ...
     *   bodyProperties?: array<string, mixed>,
     * } $options
     * @return ?GetGroupMembersResponseContent
     * @throws Auth0Exception
     * @throws Auth0ApiException
     */
    private function _get(string $id, GetGroupMembersRequestParameters $request = new GetGroupMembersRequestParameters(), ?array $options = null): ?GetGroupMembersResponseContent
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return GetGroupMembersResponseContent::fromJson($json);
            }
