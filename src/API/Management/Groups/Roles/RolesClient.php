...
use Auth0\SDK\API\Management\Types\ListGroupRolesResponseContent;
...
class RolesClient implements RolesClientInterface
{
    ...
    public function list(string $id, ListGroupRolesRequestParameters $request = new ListGroupRolesRequestParameters(), ?array $options = null): Pager
    {
        return new CursorPager(
            request: $request,
            getNextPage: fn (ListGroupRolesRequestParameters $request) => $this->_list($id, $request, $options),
            setCursor: function (ListGroupRolesRequestParameters $request, ?string $cursor) {
                $request->setFrom($cursor);
            },
            /* @phpstan-ignore-next-line */
            getNextCursor: fn (?ListGroupRolesResponseContent $response) => $response?->getNext() ?? null,
            /* @phpstan-ignore-next-line */
            getItems: fn (?ListGroupRolesResponseContent $response) => $response?->getRoles() ?? [],
        );
    }
     ...
     *   bodyProperties?: array<string, mixed>,
     * } $options
     * @return ?ListGroupRolesResponseContent
     * @throws Auth0Exception
     * @throws Auth0ApiException
     */
    private function _list(string $id, ListGroupRolesRequestParameters $request = new ListGroupRolesRequestParameters(), ?array $options = null): ?ListGroupRolesResponseContent
    {
        $options = array_merge($this->options, $options ?? []);
        $query = [];
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListGroupRolesResponseContent::fromJson($json);
            }
