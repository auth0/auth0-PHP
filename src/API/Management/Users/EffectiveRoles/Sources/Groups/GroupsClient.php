...
use Auth0\SDK\API\Management\Types\ListUserRoleSourceGroupsResponseContent;
...
class GroupsClient implements GroupsClientInterface
{
    ...
    public function list(string $id, ListUserRoleSourceGroupsRequestParameters $request, ?array $options = null): Pager
    {
        return new CursorPager(
            request: $request,
            getNextPage: fn (ListUserRoleSourceGroupsRequestParameters $request) => $this->_list($id, $request, $options),
            setCursor: function (ListUserRoleSourceGroupsRequestParameters $request, ?string $cursor) {
                $request->setFrom($cursor);
            },
            /* @phpstan-ignore-next-line */
            getNextCursor: fn (?ListUserRoleSourceGroupsResponseContent $response) => $response?->getNext() ?? null,
            /* @phpstan-ignore-next-line */
            getItems: fn (?ListUserRoleSourceGroupsResponseContent $response) => $response?->getGroups() ?? [],
        );
    }
     ...
     *   bodyProperties?: array<string, mixed>,
     * } $options
     * @return ?ListUserRoleSourceGroupsResponseContent
     * @throws Auth0Exception
     * @throws Auth0ApiException
     */
    private function _list(string $id, ListUserRoleSourceGroupsRequestParameters $request, ?array $options = null): ?ListUserRoleSourceGroupsResponseContent
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListUserRoleSourceGroupsResponseContent::fromJson($json);
            }
