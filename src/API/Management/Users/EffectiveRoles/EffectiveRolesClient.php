...
use Auth0\SDK\API\Management\Users\EffectiveRoles\Sources\SourcesClient;
...
use Auth0\SDK\API\Management\Types\ListUserEffectiveRolesResponseContent;
...
class EffectiveRolesClient implements EffectiveRolesClientInterface
{
    ...
    public function list(string $id, ListUserEffectiveRolesRequestParameters $request = new ListUserEffectiveRolesRequestParameters(), ?array $options = null): Pager
    {
        return new CursorPager(
            request: $request,
            getNextPage: fn (ListUserEffectiveRolesRequestParameters $request) => $this->_list($id, $request, $options),
            setCursor: function (ListUserEffectiveRolesRequestParameters $request, ?string $cursor) {
                $request->setFrom($cursor);
            },
            /* @phpstan-ignore-next-line */
            getNextCursor: fn (?ListUserEffectiveRolesResponseContent $response) => $response?->getNext() ?? null,
            /* @phpstan-ignore-next-line */
            getItems: fn (?ListUserEffectiveRolesResponseContent $response) => $response?->getRoles() ?? [],
        );
    }
     ...
     *   bodyProperties?: array<string, mixed>,
     * } $options
     * @return ?ListUserEffectiveRolesResponseContent
     * @throws Auth0Exception
     * @throws Auth0ApiException
     */
    private function _list(string $id, ListUserEffectiveRolesRequestParameters $request = new ListUserEffectiveRolesRequestParameters(), ?array $options = null): ?ListUserEffectiveRolesResponseContent
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    ...
                    path: "users/" . RawClient::encodePathParam($id) . "/effective-roles",
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListUserEffectiveRolesResponseContent::fromJson($json);
            }
